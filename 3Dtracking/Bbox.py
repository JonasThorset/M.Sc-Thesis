...
class Bbox:
    ...
    def drawBbox(self, frame):
        print(self.x, self.y)
        cv2.putText(frame, self.text, (int(self.x), int(self.y - 5)), cv2.FONT_HERSHEY_SIMPLEX, fontScale = 0.5, color = self.color, thickness = 1)
        cv2.rectangle(frame, (int(self.x), int(self.y)), (int(self.x+self.w), int(self.y+self.h)), color = self.color, thickness = 2, lineType = 1 )
    ...
    def addSlack(self, slack = 10):
        self.x -= slack
        self.y -= slack
        self.w += 2*slack
        self.h += 2*slack
    
    def isInBbox(self, pt, slack = 10):
        x = pt[0]
        y = pt[1]
        return (self.x -slack <= x <= self.x + self.w +2*slack) and (self.y -slack <= y <= self.y + self.h + 2*slack)
